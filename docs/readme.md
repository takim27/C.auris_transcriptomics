...
> mkdir -p STAR_index
STAR Indexing script: star_index.sh
Then ran - dos2unix star_index.sh (converting file star_align.sh to Unix format or else it show error).
Submit to HPC terminal sbatch star_index.sh
Expected files type:
•	Genome
•	SA
•	SAindex
...
> conda activate auris_core_v2
Went to the folder where all the fastq files are and ran: fastqc -t 8 -o . *.fastq.gz
Then multiqc . -o multiqc_raw
Check the output from multiqc - multiqc_report.html 

Step 8: Trimming by cutadapt 
Cutadapt script: cutadapt_quantseq.sh
...
/project/bishalab/msarker/c.auris_new/fastqc_trimmed/multiqc_trimmed.html
Step 10: STAR alignment
> conda activate auris_core_v2
> mkdir -p /project/bishalab/msarker/c.auris_new/STAR_alignment
Created the STAR_alignment.sh
...
Step 15: GFFCompare
Ran GFFCompare against the same GENCODE v48 reference.
First confirmed the merged GTF contains only the 25 expected chromosomes:
> awk '!/^#/ {print $1}' /project/bishalab/msarker/c.auris_new/stringtie_merge/stringtie_merged.gtf | sort -u
Created the output directory: mkdir -p /project/bishalab/msarker/c.auris_new/gffcompare
Created gffcompare.sh
Submitted to slurm: sbatch gffcompare.sh
Expected file types:
...
Extracted the primary u,x transcripts from the GFFCompare annotated GTF
1. Extract u+x
...
Saved this as cpc2_scan.sh:
Submit:
> dos2unix cpc2_scan.sh
> sbatch cpc2_scan.sh
The final CPC2 output will be:
> /project/bishalab/msarker/c.auris_new/lncrna_filtering/05_cpc2/results/cpc2_results.txt
...
Create screening_featurecounts.sh
Submit:
...
Step 28: getting the transcript ids gtf file for the 613 expression-passing loci 
...
In IGV, selected Genomes Human hg38.
...
MSTRG.36939.1 appears twice because it was evaluated against two coding genes. The six strict catalogue exclusions are:
•	MSTRG.1803.1
•	MSTRG.17016.1
•	MSTRG.22578.15
•	MSTRG.34694.1
...
All the downstream analysis of UpSet overlapping, Enrichment, Co-expression, temporal interaction analysis, cis-proximity and co-expression base network was done and plotted in those 2 data files. 
